...
def chamadosmesaberto():
    ...
    criancas =  Crianca.objects.filter(excluido=False, desativado=False, data_inicio__gte=data,status=False)
    return len(criancas)
...
@register.simple_tag(name='turma_destinada')
def turma_destinada(id):
    crianca = Crianca.objects.filter(id=id).last()
    if crianca:
        return calculoTurma(crianca.data_nasc)
    return ''
